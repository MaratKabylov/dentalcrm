...
import { listCrmAssignees, listCrmBranches, listPatientSources } from "@/modules/crm/repository";
import { requirePermission } from "@/modules/organizations/repository";

export default async function NewLeadPage() {
  await requirePermission("crm.manage");
  const [sources, branches, assignees] = await Promise.all([
    listPatientSources(),
    listCrmBranches(),
    listCrmAssignees(),
  ]);
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Card className="p-5 sm:p-7"><LeadForm sources={sources} branches={branches} assignees={assignees} /></Card>
    </div>
  );
}
