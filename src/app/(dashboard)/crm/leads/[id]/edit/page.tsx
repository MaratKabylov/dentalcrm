...
import { getLead, listCrmAssignees, listCrmBranches, listPatientSources } from "@/modules/crm/repository";
import { requirePermission } from "@/modules/organizations/repository";

export default async function EditLeadPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await requirePermission("crm.manage");
  const [lead, sources, branches, assignees] = await Promise.all([
    getLead(id), listPatientSources(true), listCrmBranches(), listCrmAssignees(),
  ]);
  if (!lead) notFound();
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Card className="p-5 sm:p-7"><LeadForm lead={lead} sources={sources} branches={branches} assignees={assignees} /></Card>
    </div>
  );
}
